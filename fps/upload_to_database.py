...
cnx = pymysql.connect(user='root', password='root', host='localhost', database='upt_mysql_db')
...
# Header, Address and Package Identifier
...
def img2Tz(buf):
    # Generate character file from the original finger image in imageBuffer
    # and store the file in CharBuffer1 or CharBuffer2.
    data = [0x2, buf]
    writePacket(data)
    s = readPacket()
    return s[4]
...
def regModel():
    # Combine information of character files from CharBuffer1 and CharBuffer2 and generate a  \n"
    # template which is stroed back in both CharBuffer1 and CharBuffer2.")
    data = [0x5]
    writePacket(data)
    s = readPacket()
    return s[4]
...
while name != 'done':
    ...
    print("Put finger again", sys.stdout.flush())
